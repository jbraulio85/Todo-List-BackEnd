...
exports.addTask = async(req, res)=>{
    try{
        ...
        await newTaks.save;
        return res.send({message: 'Actividad guardada satisfactoriamente'});
    }catch(err){
        console.error(err);
...
exports.updateTask = async(req, res)=>{
    try{
        ...
        let data = req.bodyM;
        let updatedTask = Task.findOneAndUpdate(
            {_id: task},
            data,
            {new: true}
        )
...
exports.deleteTask = async(req, res)=>{
    try{
        ...
        let deletedTask = await Tasks.findOneAndDelete({_id: idTask});
